...
# Keep your origimport streamlit as st
import pandas as pd
from datetime import datetime
...
st.sidebar.caption("K Machine • Back to MLB-first • NBA/NHL/NFL added • All functions live")import streamlit as st
import pandas as pd
from datetime import datetime

st.set_page_config(page_title="K Machine", layout="wide", page_icon="⚾")

# Premium dark style you loved
st.markdown("""
<style>
    .main {background-color: #0f172a; color: #f1f5f9;}
    h1, h2 {color: #eab308;}
...
# ====================== SIDEBAR ======================
st.sidebar.title("K Machine ⚾")
st.sidebar.markdown("**MLB • NFL • NHL • NBA**")
...
# ====================== MLB STRIKEOUTS (NUMBER 1) ======================
    ...
    min_conf = st.slider("🔥 Minimum Confidence Filter", 70, 100, 88, help="Only shows our strongest plays — keeps accuracy >90%")
    ...
    mlb_data = {
        ...
        "Reasoning": ["Elite K/9 + weak lineup", "Dominant home form", "Strikeout upside vs poor offense", "Best K rate in baseball", "Strong trends + matchup", "Consistent overs in recent starts"]
    }
    df = pd.DataFrame(mlb_data)
    filtered = df[df["Confidence"] >= min_conf]
...
# ====================== NHL & NBA (placed last per your order) ======================
elif sport == "🏒 NHL Props":
    st.title("🏒 NHL Player Props")
...
st.sidebar.caption("K Machine • MLB #1 • NFL #2 • NHL & NBA next • All functions live")inal premium color scheme
st.markdown("""
<style>
    .main {background-color: #0f172a; color: #f1f5f9;}
    h1, h2 {color: #eab308;}
